...
__global__ void MatrixMultiplyKernel(
   ...
   */
   ...
   int batch = blockIdx.z;
    ...
    int m = out_shape[1];   // Number of rows
    int p = out_shape[2];   // Cols
    int n = a_shape[2];
    ...
    if (row < m && col < p) {
        ...
        for (int k = 0; k < n; k++) {
            ...
            // independently calc the positions for a and b
            ...
            // just do the multiply and accumulate
...
__global__ void mapKernel(
   ...
   */
    ...
    // 6. Apply the unary function to the input element and write the output to the out memory
        // Grid-stride over all output elements

    int total_number_of_threads = blockDim.x * gridDim.x;

    for (int index = blockIdx.x * blockDim.x + threadIdx.x;
         index < out_size;
         index += total_number_of_threads) {

        // We gotta take this absolute index and convert to the
        // set of multi dimensional indices
        to_index(index, out_shape, out_index, shape_size);

        // Broadcast to set the in_index correctly
        broadcast_index(out_index, out_shape, in_shape,in_index, shape_size, shape_size);

        // Turn indices into storage positions using strides
        int i_in  = index_to_position(in_index,  in_strides,  shape_size);
        int i_out = index_to_position(out_index, out_strides, shape_size);

        out[i_out] = fn(fn_id, in_storage[i_in]);
    }
   ...
   */
    ...
    for (int index = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        for (int r = 0; r < a_shape[reduce_dim]; r++) {
            ...
            // Thats the trick, give the accumulator back to fn!
...
__global__ void zipKernel(
   ...
   */
    ...
    for (int index = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        // We gotta take this absolute index and convert to the
        // set of multi dimensional indices
        to_index(index, out_shape, out_index, out_shape_size);

        // Broadcast to set the a_index correctly
        ...
        // Turn indices into storage positions using strides
        int i_a  = index_to_position(a_index,  a_strides,  a_shape_size);
        int i_b  = index_to_position(b_index,  b_strides,  b_shape_size);

        int i_out = index_to_position(out_index, out_strides, out_shape_size);

        out[i_out] = fn(fn_id, a_storage[i_a], b_storage[i_b]);
    }
    /// END ASSIGN2_2
}
