...
def cross_entropy_loss(out, y):
    ...
    # where p is the predicted probability (out after sigmoid)
    
    # Create ones tensor with same shape as y
    ones = minitorch.tensor_functions.ones(y.shape, backend=y.backend)
    
    # Calculate binary cross entropy.  This is it:
...
class Linear(minitorch.Module):
    ...
    def forward(self, x):
        ...
        if len(x.shape) == 3:
            # If input is 3D (batch, 1, in_size), squeeze the middle dimension
            batch, _, in_size = x.shape
        ...
        # Reshape weights to ensure it's (in_size, out_size)
        weights = self.weights.value.view(in_size, self.out_size)
        
        # Matmul: (batch, in_size) @ (in_size, out_size) = (batch, out_size)
        output = x @ weights
...
# A simple cosine annealing scheduler,this really seemed to help
# as without it I couldn't get about 70% validation, but with this
# I got to 78%. Taking 11-785 definitely showed me the importance of 
...
class CosineAnnealingScheduler:
    def __init__(self, optimizer, T_max, eta_min=0.0):
        ...
        self.t = 0
        self.lr_initial = optimizer.lr

    def step(self):
        lr = self.eta_min + (self.lr_initial - self.eta_min) * \
             (1 + math.cos(math.pi * self.t / self.T_max)) / 2

        # just update the optimizaer directly, these optimizers are simple
...
class SentenceSentimentTrain:
    ...
    def train(
        ...
        scheduler = CosineAnnealingScheduler(optim, T_max=100, eta_min=1e-3)
        losses = []
        train_accuracy = []
        validation_accuracy = []
        for epoch in range(1, max_epochs + 1):
            ...
            for batch_num, example_num in enumerate(
                ...
                batch_x = X_train[example_num:batch_end]
                batch_y = y_train[example_num:batch_end]
                
                # Create tensors with requires_grad=True
                x = minitorch.tensor(batch_x, backend=BACKEND, requires_grad=True)
                y = minitorch.tensor(batch_y, backend=BACKEND, requires_grad=True)
            ...
            if data_val is not None:
                ...
                # Create validation tensors (using SimpleOps backend, not CUDA)
                x_val = minitorch.tensor(X_val, backend=BACKEND, requires_grad=False)
                
                # Get model output (no gradients needed for validation)
                out_val = model(x_val)
